...
mucinous_clinical = subset(clinical, histological_type %in% "Stomach, Intestinal Adenocarcinoma, Mucinous Type")
GeneMatrix_mucinous <- subset(GeneMatrix_MGI, select = substr(colnames(GeneMatrix_MGI),1,12) %in% mucinous_clinical$bcr_patient_barcode)
dim(GeneMatrix_mucinous)
...
# mucinous
mucinous_phenotype <- as.data.frame(colnames(GeneMatrix_mucinous))
mucinous_phenotype$Disease <- 'mucinous'
mucinous_phenotype$Status <- 1
colnames(mucinous_phenotype)<-  c("Patient", 'Disease', "Status")
Phenotype_normal_mucinous <- rbind(Normal_phenotype, mucinous_phenotype)
write.table(Phenotype_normal_mucinous, file="D:/KP/EKP_RKP/Scissor/Phenotype_normal_mucinous.csv", sep = ",", row.names=FALSE)

# 3. match gene matrix and phenotype list
# only include patients that also exist in the phenotype list
GeneMatrix_normal_mucinous <- subset(GeneMatrix_MGI, select = colnames(GeneMatrix_MGI) %in% Phenotype_normal_mucinous$Patient)
patient = as.data.frame(colnames(GeneMatrix_normal_mucinous))

duplicated(Phenotype_normal_mucinous$Patient)
Phenotype_normal_mucinous_1 <- Phenotype_normal_mucinous[!duplicated(Phenotype_normal_mucinous$Patient),]
Phenotype_normal_mucinous <- Phenotype_normal_mucinous_1

# make gene matrix and phenotype list the same order
GeneMatrix_normal_mucinous <- GeneMatrix_normal_mucinous[ , order(match(colnames(GeneMatrix_normal_mucinous), Phenotype_normal_mucinous$Patient))]
all(colnames(GeneMatrix_normal_mucinous) == Phenotype_normal_mucinous$Patient)

# make the sample id annotated phenotype list
rownames(Phenotype_normal_mucinous) <- Phenotype_normal_mucinous$Patient
Phenotype_normal_mucinous_input = Phenotype_normal_mucinous[,!(colnames(Phenotype_normal_mucinous) %in% c('Patient','Disease'))]
Phenotype_normal_mucinous_input
names(Phenotype_normal_mucinous_input) = rownames(Phenotype_normal_mucinous)
Phenotype_normal_mucinous_input
table(Phenotype_normal_mucinous_input)
...
# EKP vs RKP
phenotype <- Phenotype_normal_mucinous_input
tag <- c('Normal', 'mucinous')
GeneMatrix_normal_mucinous_input  <- as.matrix(GeneMatrix_normal_mucinous)

infos <- Scissor(bulk_dataset=GeneMatrix_normal_mucinous_input, sc_dataset=t.rna.df, phenotype=phenotype, tag = tag, alpha = 0.32, family = "binomial", Save_file = "D:/KP/EKP_RKP/Scissor/normal_mucinous.RData")
...
write.table(as.data.frame(Scissor_select), "D:/KP/EKP_RKP/Scissor/Scissor_normal_mucinous_a0.32.csv", row.names = T, col.names = T, quote = F, sep = ",")    # export meta.data of sc_dataset, you can check Scissor information

infos <- Scissor(bulk_dataset=GeneMatrix_normal_mucinous_input, sc_dataset=t.rna.df, phenotype=phenotype, tag = tag, alpha = 0.05, family = "binomial", Save_file = "D:/KP/EKP_RKP/Scissor/normal_mucinous_a0.05.RData")
Scissor_select <- rep(0, ncol(t.rna.df))
names(Scissor_select) <- colnames(t.rna.df)
Scissor_select[infos$Scissor_pos] <- 1
Scissor_select[infos$Scissor_neg] <- 2
write.table(as.data.frame(Scissor_select), "D:/KP/EKP_RKP/Scissor/Scissor_normal_mucinous_a0.05.csv", row.names = T, col.names = T, quote = F, sep = ",")    # export meta.data of sc_dataset, you can check Scissor information

infos <- Scissor(bulk_dataset=GeneMatrix_normal_mucinous_input, sc_dataset=t.rna.df, phenotype=phenotype, tag = tag, alpha = 0.01, family = "binomial", Save_file = "D:/KP/EKP_RKP/Scissor/normal_mucinous_a0.01.RData")
Scissor_select <- rep(0, ncol(t.rna.df))
names(Scissor_select) <- colnames(t.rna.df)
Scissor_select[infos$Scissor_pos] <- 1
Scissor_select[infos$Scissor_neg] <- 2
write.table(as.data.frame(Scissor_select), "D:/KP/EKP_RKP/Scissor/Scissor_normal_mucinous_a0.01.csv", row.names = T, col.names = T, quote = F, sep = ",")    # export meta.data of sc_dataset, you can check Scissor information
