...
setwd("D:/KP/EKP_Aqp5..cell/fgsea")
######################################################################################################
...
DEG <- read.csv("D:/KP/EKP_Aqp5..cell/DEG_by_celltype.csv", sep = ',')
DEG_1 <- DEG %>% select(Aqp5..cell_names, Aqp5..cell_score, Aqp5..cell_pvals)
DEG_1 %>% filter(Aqp5..cell_pvals < 0.05)
DEG_1$gene <- DEG_1$Aqp5..cell_names
...
fwrite(fgseaResTidy, file="Aqp5..cell_GSEA_results_KEGG.csv", sep=",", sep2=c("", " ", ""))
...
fwrite(fgseaResTidy_2, file="Aqp5..cell_GSEA_results_REACTOME.csv", sep=",", sep2=c("", " ", ""))
fgseaResTidy_2 = read.csv('Aqp5..cell_GSEA_results_REACTOME.csv', sep=",")
...
fwrite(fgseaResTidy_5, file="Aqp5..cell_GSEA_results_GOBP.csv", sep=",", sep2=c("", " ", ""))
fgseaResTidy_5 = read.csv("Aqp5..cell_GSEA_results_GOBP.csv", sep = ",")
...
ggplot(fgseaResTidy_5, aes(reorder(pathway, NES), NES)) +
  ...
  coord_flip() +
  labs(x="Pathway", y="Normalized Enrichment Score",
       title="Aqp5..cellvs.others GOBP pathways") + 
  theme_minimal()
