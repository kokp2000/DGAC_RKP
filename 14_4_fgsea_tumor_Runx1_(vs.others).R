...
setwd("D:/KP/EKP_Runx1..stem_1/fgsea")
######################################################################################################
####################### fgsea using Wilcoxon gene rank list from Scanpy ##############################
######################################################################################################

###################### Runx1..stem_1 to others ################################################################

### KEGG ###

DEG <- read.csv("D:/KP/EKP_RKP/DEG_by_celltype.csv", sep = ',')
DEG_1 <- DEG %>% select(Runx1..stem_1_names, Runx1..stem_1_score, Runx1..stem_1_pvals)
DEG_1 %>% filter(Runx1..stem_1_pvals < 0.05)
DEG_1$gene <- DEG_1$Runx1..stem_1_names
...
ranks_1 <- DEG_1$Runx1..stem_1_score
#ranks_1 <- DEG_1$rank
...
fwrite(fgseaResTidy, file="Runx1..stem_1_GSEA_results_KEGG.csv", sep=",", sep2=c("", " ", ""))
...
fwrite(fgseaResTidy_2, file="Runx1..stem_1_GSEA_results_REACTOME.csv", sep=",", sep2=c("", " ", ""))
fgseaResTidy_2 = read.csv('Runx1..stem_1_GSEA_results_REACTOME.csv', sep=",")
...
fwrite(fgseaResTidy_5, file="Runx1..stem_1_GSEA_results_GOBP.csv", sep=",", sep2=c("", " ", ""))
fgseaResTidy_5 = read.csv("Runx1..stem_1_GSEA_results_GOBP.csv", sep = ",")
...
ggplot(fgseaResTidy_5, aes(reorder(pathway, NES), NES)) +
  ...
  coord_flip() +
  labs(x="Pathway", y="Normalized Enrichment Score",
       title="Runx1..stem_1vs.others GOBP pathways") + 
  theme_minimal()
